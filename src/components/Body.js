...
const Body = () => {
  ...
  const fetchData = async () => {
    const data = await fetch(
      "https://www.swiggy.com/dapi/restaurants/list/v5?lat=19.2993420046926&lng=72.86991979871428&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING"
    );

    const json = await data.json();

    setlist0fRestaurants(
      json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants
    ); //array jaa rha hai list0fRestaurants mein
    setfilteredRestaurants(
      json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants
    ); //Will be using this manipulation like search filtered
    console.log(
      json?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants
    );
  };
      ...
      <div className="filter flex">
        <div className="search m-4 p-4">
          ...
          <button
            ...
            onClick={() => {
              let searchFiltered = listOfRestaurants.filter((res) =>
